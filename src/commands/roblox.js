...
module.exports = class Roblox extends Command {
    ...
    async run(message, args) {
        ...
        const user = await this.client.apis.roblox.getUserBasics(args.join(' '));
        if (user.errorMessage) return message.channel.send(`${this.lang.getEmoji('error')} ${this.lang.get('robloxNo')}`);
        const friends = await this.client.apis.roblox.getUserFriends(user.Id);
        const groups = await this.client.apis.roblox.getUserGroups(user.Id);
