...
module.exports = class CommandsManager extends Collection {
    ...
    loadCommands() {
        for (const file of fs.readdirSync(path.join(__dirname, '../commands/'))) {
            ...
            const existingCommand = this.find((c) => [...command.aliases, command.name, command.constructor.name].some((n) => [...c.aliases, c.name, c.constructor.name].includes(n)));
            if (existingCommand) this.client.log(`Commands with equal names or aliases found (${file} and ${existingCommand.name})`, true);
            const helpStrings = this.client.languages.getAll().map((l) => [l.getHelp(`${command.name}Usage`), l.getHelp(`${command.name}Description`)]).flat();
