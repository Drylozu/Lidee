...
module.exports = class Language {
    ...
    getConstant(category, property, ...args) {
        const object = this.constants[category];
    ...
    parsePermissions(permissions) {
        ...
        let output = '';
        const defaultPermissions = new Permissions(Permissions.DEFAULT).toArray();
        permissions
    ...
    parseCompleteDate(date) {
        ...
        if (date.getTime() > new Date().getTime())
            old = false;
        const milliseconds = old ? new Date() - date : date - new Date();
        const timeObj = {
    ...
    parseMemberRoles(memberRoles) {
        const everyoneRole = memberRoles.guild.roles.everyone;
        const hoistRole = memberRoles.hoist;
        const rolesParsed = memberRoles.cache
    ...
    parseMemberActivity(activities, client) {
        const customStatus = activities.find((a) => a.type === 'CUSTOM_STATUS');
        ...
        const statusEmojiGuild = customStatus && customStatus.emoji &&
    ...
    parseServerRoles(serverRoles) {
        const everyoneRole = serverRoles.guild.roles.everyone;
        const rolesParsed = serverRoles.cache
            .filter((r) => r.id !== everyoneRole.id)
            .sort((a, b) => b.position - a.position)
            .map((r) => `<@&${r.id}>`);
        if (rolesParsed.length < 1) return this.get('nothing');
    ...
    parseServerEmojis(serverEmojis) {
        ...
        const normalEmojisOut = [];
        const animatedEmojisOut = [];
