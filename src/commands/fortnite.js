...
module.exports = class Fortnite extends Command {
    ...
    async run(message, [arg, page]) {
        if (!arg) return message.channel.send(`${this.lang.getEmoji('error')} ${this.lang.get('fortniteNo')}`);
            ...
            const shop = await this.client.apis.fortnite.getDailyShop(this.lang.languageCode === 'pt' ? 'pt-BR' : this.lang.languageCode);
            const item = isNaN(page) || parseInt(page) < 1 || parseInt(page) > shop.length
                ? 1 : parseInt(page);
            const shopItem = shop[parseInt(item) - 1];
            ...
            const user = await this.client.apis.fortnite.getUserStats(arg);
