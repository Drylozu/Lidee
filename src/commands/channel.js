...
module.exports = class Channel extends Command {
    ...
    async run(message, args) {
        const channel = message.guild.channels.resolve(args[0]) || message.mentions.channels.first() || (args.join(' ').length > 2 ? message.guild.channels.cache.sort((a, b) => a.name.localeCompare(b.name)).find((c) => c.name.toLowerCase().includes(args.join(' ').toLowerCase())) : null) || message.channel;
        const alloweds = [];
        const denieds = [];
        message.guild.roles.cache
            .sort((a, b) => b.rawPosition - a.rawPosition)
            .forEach((role) => {
                const permissions = channel.permissionsFor(role.id);
