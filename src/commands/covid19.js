...
module.exports = class COVID19 extends Command {
    ...
    async run(message, args) {
        ...
        try {
            const AllUpdated = await this.client.apis.covid.getAllLastUpdate();
            const { Global, Countries } = await this.client.apis.covid.getAllStats();
            if (!country || !Countries.find((c) => c.Country.toLowerCase().includes(country.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase())))
                ...
                const covidCountry = Countries.find((c) => c.Country.toLowerCase().includes(country.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()));
                message.channel.send(new MessageEmbed()
